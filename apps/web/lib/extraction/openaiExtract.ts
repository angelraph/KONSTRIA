...
const roomLabelSchema = z.object({
  ...
  topLeftXPx: z.number(),
  topLeftYPx: z.number(),
  bottomRightXPx: z.number(),
  bottomRightYPx: z.number(),
  confidence: z.number().min(0).max(1),
});
...
- openings: doors and windows, each linked to the wall it sits on via wallTempId. Report the two pixel endpoints of the gap itself (startXPx/startYPx to endXPx/endYPx along the wall), not just a center point, so its real-world width can be computed later.
- roomLabels: any room name text printed on the plan (e.g. "LIVING ROOM", "BEDROOM 1"), with the pixel coordinates of that room's approximate bounding rectangle (topLeftXPx/topLeftYPx to bottomRightXPx/bottomRightYPx, i.e. the enclosing walls), so its real-world area can be computed later. Estimate the rectangle from the visible walls around the label, not just the text itself.
